...
def exponential_lambda_return(reward, value, pcont, bootstrap, lambda_, beta_, axis):
    # Setting lambda=1 gives a discounted Monte Carlo return.
    # Setting lambda=0 gives a fixed 1-step return.
    # assert reward.shape.ndims == value.shape.ndims, (reward.shape, value.shape)
    assert len(reward.shape) == len(value.shape), (reward.shape, value.shape)
    ...
    if bootstrap is None:
        bootstrap = torch.zeros_like(value[-1])
    next_values = torch.cat([value[1:], bootstrap[None]], 0)
    inputs = beta_ * reward + pcont * torch.log(1e-15 + torch.relu(next_values)) * (1 - lambda_)
    returns = static_scan_for_lambda_return(
        lambda agg, cur0, cur1: torch.exp(cur0 + cur1 * lambda_ * torch.log(1e-15 + torch.relu(agg))),
