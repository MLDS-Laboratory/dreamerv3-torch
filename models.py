...
class SwitchBehavior(nn.Module):
    ...
    def actor(self, feat, **kwargs):
        ...
        risk_averse_action = self._risk_averse_behavior.actor(feat, **kwargs)
        return risk_seeking_action, risk_averse_action
...
class RiskSensitiveImagBehavior(nn.Module):
    def __init__(self, config, world_model, risk_averse=False):
        super(RiskSensitiveImagBehavior, self).__init__()
        self._use_amp = True if config.precision == 16 else False
        self._config = config
        self._world_model = world_model

        self._mvpi_lambda = self._config.mvpi_lambda
        self._beta = self._config.beta
        self._mg_lambda = self._config.mg_lambda
        if config.switch:
            if risk_averse:
                self._beta *= -1
            else:
        ...
        with tools.RequiresGrad(self.actor):
            with torch.cuda.amp.autocast(self._use_amp):
                ...
                if self._config.algorithm == 'mg':
                    ...
                    actor_loss += self._mg_lambda * gini_loss
                    metrics.update(mets)
    ...
    def _compute_target(self, imag_feat, imag_state, reward):
        ...
        if self._config.algorithm == 'mvpi':
            y = reward.mean(1, keepdim=True)
            reward = reward - self._mvpi_lambda * reward ** 2 + 2 * self._mvpi_lambda * reward * y
